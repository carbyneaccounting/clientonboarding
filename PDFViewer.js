...
// No dependencies in your HTML. This module creates/removes its own DOM.
...
function _css(){
  ...
  /* PDF.js viewer defaults need these */
  .pdfViewer .page{
    margin:0 0 12px 0;
    border-radius:8px;
    overflow:hidden;
...
function _buildOverlay(){
  ...
  // wire controls after viewer is created in open()

  // Download/Print handlers (set per open())
  downloadBtn.addEventListener("click", () => {
    if (!_state.pdfDoc) return;
    _state.pdfDoc.getData().then((data) => {
  ...
  _state.pageNumber.addEventListener("change", () => {
    if (!_state.pdfViewer) return;
  ...
  _state.scaleSelect.addEventListener("change", () => {
    if (!_state.pdfViewer) return;
...
export async function open(pdfUrlOrBytes, opts = {}){
  await _loadPdfJs();
  _buildOverlay();
  ...
  // Let overlay capture focus
  _state.overlay.tabIndex = -1;
  _state.overlay.focus();

  const { EventBus, PDFViewer, PDFLinkService, PDFFindController } = _state.pdfjsViewer;
  ...
  _state.pdfViewer = new PDFViewer({
    container: _state.overlay.querySelector(".cv-pdf-main"),
  ...
  _state.eventBus.on("pagesinit", () => {
    _state.pdfViewer.currentScaleValue = "page-width";
    _syncPageUi();
  });

  _state.eventBus.on("pagechanging", () => _syncPageUi());

  const loadingTask = _state.pdfjsLib.getDocument(pdfUrlOrBytes);
